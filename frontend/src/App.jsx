...
import { Clock, Check, X, Download, LogOut, ListChecks, FileEdit, FileSpreadsheet, ClipboardList, Plus, AlertCircle, Users, Pencil, MapPin, Building2, Trash2 } from "lucide-react";
import { api, setToken, getToken, downloadBlob } from "./api.js";
import logo from "./assets/inducat-logo.png";

const LANGS = ["hu", "en", "de", "ro", "tr"];
const ALLOWANCE_STEPS = Array.from({ length: 21 }, (_, i) => i * 5); // 0, 5, 10, ..., 100
...
const TRANSLATIONS = {
  hu: {
    ...
    nav: { dashboard: "Kezdőlap", newEntry: "Új bejegyzés", approvals: "Jóváhagyás", corrections: "Javítás", export: "Export", auditLog: "Auditnapló", users: "Felhasználók", locations: "Telephelyek", departments: "Részlegek", entries: "Bejegyzések", logout: "Kilépés" },
    ...
    fields: { date: "Dátum", start: "Kezdés", end: "Végzés", hours: "Ledolgozott óra", shift: "Műszakkód", location: "Munkavégzés helye", comment: "Megjegyzés", extraAllowance: "Extra pótlék (%)", extraAllowanceHours: "Pótlék óraszáma", projectNumber: "Projektszám", department: "Részleg", perDiem: "Napidíj (X)" },
    ...
    exportView: { title: "Havi elszámolási export", month: "Hónap", periodFrom: "Időszak kezdete", periodTo: "Időszak vége", rangeInvalid: "A kezdő hónap nem lehet később, mint a záró hónap.", group: "Csoport / vezető", allGroups: "Minden csoport", name: "Név", expected: "Havi várható munkaidő", worked: "Ledolgozott órák", difference: "Különbség", perDiemDays: "Napidíjas napok", extraAllowance: "Extra pótlék (súlyozott átl. %)", extraAllowanceHours: "Pótlékos órák", approver: "Jóváhagyó neve", lastMod: "Utolsó módosítás", lastModBy: "Utolsó módosítást végző", noneYet: "—" },
  ...
  en: {
    ...
    nav: { dashboard: "Dashboard", newEntry: "New entry", approvals: "Approvals", corrections: "Corrections", export: "Export", auditLog: "Audit log", users: "Users", locations: "Locations", departments: "Departments", entries: "Entries", logout: "Sign out" },
    ...
    fields: { date: "Date", start: "Start", end: "End", hours: "Worked hours", shift: "Shift code", location: "Work location", comment: "Comment", extraAllowance: "Extra allowance (%)", extraAllowanceHours: "Allowance hours", projectNumber: "Project number", department: "Department", perDiem: "Per diem (X)" },
  ...
  de: {
    ...
    nav: { dashboard: "Übersicht", newEntry: "Neuer Eintrag", approvals: "Freigaben", corrections: "Korrekturen", export: "Export", auditLog: "Prüfprotokoll", users: "Benutzer", locations: "Standorte", departments: "Abteilungen", entries: "Einträge", logout: "Abmelden" },
    ...
    fields: { date: "Datum", start: "Beginn", end: "Ende", hours: "Geleistete Stunden", shift: "Schichtcode", location: "Arbeitsort", comment: "Bemerkung", extraAllowance: "Zusätzliche Zulage (%)", extraAllowanceHours: "Zulagenstunden", projectNumber: "Projektnummer", department: "Abteilung", perDiem: "Tagegeld (X)" },
    ...
    exportView: { title: "Monatlicher Abrechnungsexport", month: "Monat", periodFrom: "Zeitraum von", periodTo: "Zeitraum bis", rangeInvalid: "Der Startmonat darf nicht nach dem Endmonat liegen.", group: "Gruppe / Vorgesetzter", allGroups: "Alle Gruppen", name: "Name", expected: "Monatliche Sollstunden", worked: "Geleistete Stunden", difference: "Differenz", perDiemDays: "Tagegeld-Tage", extraAllowance: "Zusätzliche Zulage (gew. Ø %)", extraAllowanceHours: "Zulagenstunden", approver: "Name des Freigebers", lastMod: "Letzte Änderung", lastModBy: "Geändert von", noneYet: "—" },
  ...
  ro: {
    ...
    nav: { dashboard: "Panou", newEntry: "Înregistrare nouă", approvals: "Aprobări", corrections: "Corecții", export: "Export", auditLog: "Jurnal audit", users: "Utilizatori", locations: "Locații", departments: "Departamente", entries: "Înregistrări", logout: "Deconectare" },
    ...
    fields: { date: "Data", start: "Început", end: "Sfârșit", hours: "Ore lucrate", shift: "Cod tură", location: "Locul de muncă", comment: "Comentariu", extraAllowance: "Indemnizație suplimentară (%)", extraAllowanceHours: "Ore cu indemnizație", projectNumber: "Număr proiect", department: "Departament", perDiem: "Diurnă (X)" },
    ...
    exportView: { title: "Export lunar pentru salarizare", month: "Lună", periodFrom: "Perioada de la", periodTo: "Perioada până la", rangeInvalid: "Luna de început nu poate fi ulterioară lunii de sfârșit.", group: "Grup / șef", allGroups: "Toate grupurile", name: "Nume", expected: "Ore lunare așteptate", worked: "Ore lucrate", difference: "Diferență", perDiemDays: "Zile cu diurnă", extraAllowance: "Indemnizație suplimentară (medie ponderată %)", extraAllowanceHours: "Ore cu indemnizație", approver: "Numele aprobatorului", lastMod: "Ultima modificare", lastModBy: "Modificat de", noneYet: "—" },
  ...
  tr: {
    ...
    nav: { dashboard: "Panel", newEntry: "Yeni kayıt", approvals: "Onaylar", corrections: "Düzeltmeler", export: "Dışa aktar", auditLog: "Denetim günlüğü", users: "Kullanıcılar", locations: "Lokasyonlar", departments: "Departmanlar", entries: "Kayıtlar", logout: "Çıkış yap" },
    ...
    fields: { date: "Tarih", start: "Başlangıç", end: "Bitiş", hours: "Çalışılan saat", shift: "Vardiya kodu", location: "Çalışma yeri", comment: "Not", extraAllowance: "Ek ödenek (%)", extraAllowanceHours: "Ek ödenek saati", projectNumber: "Proje numarası", department: "Departman", perDiem: "Yevmiye (X)" },
    ...
    exportView: { title: "Aylık bordro dışa aktarımı", month: "Ay", periodFrom: "Dönem başlangıcı", periodTo: "Dönem bitişi", rangeInvalid: "Başlangıç ayı bitiş ayından sonra olamaz.", group: "Grup / yönetici", allGroups: "Tüm gruplar", name: "İsim", expected: "Aylık beklenen saat", worked: "Çalışılan saat", difference: "Fark", perDiemDays: "Yevmiye günleri", extraAllowance: "Ek ödenek (ağırlıklı ort. %)", extraAllowanceHours: "Ek ödenek saati", approver: "Onaylayan adı", lastMod: "Son değişiklik", lastModBy: "Değiştiren", noneYet: "—" },
...
// A super admin mindenben admin jogokkal bír, plusz törölhet bejegyzéseket.
...
function EntryForm({ t, locations, departments, onCancel, onSave, onSubmit, busy }) {
  ...
  const dept = departments[deptIdx] || { name: "" };
  const valid = form.date && form.start && form.end && locations.length > 0 && departments.length > 0;
  ...
  function payload() {
    return {
      ...
      extraAllowanceHours: Math.min(hours, Math.max(0, Number(form.extraAllowanceHours) || 0)),
      country: loc.country,
      city: loc.city,
      plant: loc.plant,
  ...
  return (
    <div className="panel">
      <div className="form-grid">
        ...
        <div className="field">
          <label className="field-label">{t("fields.department")}</label>
          <select className="input" value={deptIdx} onChange={(e) => setDeptIdx(Number(e.target.value))}>
            {departments.map((d, i) => (
        ...
        <div className="field">
          <label className="field-label">{t("fields.projectNumber")}</label>
          <input className="input" value={form.projectNumber} onChange={(e) => set("projectNumber", e.target.value)} />
        </div>
        <div className="field">
          <label className="field-label">{t("fields.extraAllowance")}</label>
          <select className="input" value={form.extraAllowance} onChange={(e) => set("extraAllowance", e.target.value)}>
            {ALLOWANCE_STEPS.map((p) => (
              <option key={p} value={p}>{p}%</option>
            ))}
          </select>
        </div>
        {Number(form.extraAllowance) > 0 && (
          <div className="field">
            <label className="field-label">{t("fields.extraAllowanceHours")}</label>
            <input type="number" min="0" max={hours} step="0.5" className="input" value={form.extraAllowanceHours} onChange={(e) => set("extraAllowanceHours", e.target.value)} />
          </div>
        )}
        <div className="field field-checkbox">
          <label className="field-label field-label-checkbox">
            <input type="checkbox" checked={form.perDiem} onChange={(e) => set("perDiem", e.target.checked)} />
            {t("fields.perDiem")}
          </label>
        </div>
        <div className="field field-wide">
        ...
        <div className="ledger">
          {recent.map((e) => (
            <div className="ledger-row" key={e.id}>
              ...
              <div className="ledger-loc">
                {e.plant}{e.department ? ` · ${e.department}` : ""}{e.projectNumber ? ` · #${e.projectNumber}` : ""}
              </div>
              {e.extraAllowance > 0 && (
                <div className="ledger-extra">+{e.extraAllowance}% ({e.extraAllowanceHours} {t("misc.hoursShort")})</div>
              )}
              {e.perDiem && <div className="ledger-extra">{t("fields.perDiem")}</div>}
...
function ApprovalView({ t, user, entries, employees, onApprove, onReturn }) {
  ...
  const team = isAdmin ? employees.filter((e) => !isAdminRole(e.role)) : employees.filter((e) => e.supervisorId === user.id);
  ...
  function getOverride(e) {
    return overrides[e.id] ?? { percent: e.extraAllowance, hours: e.extraAllowanceHours };
  }
  ...
  function approveWithOverride(e) {
    if (!isAdmin) return onApprove(e.id);
    const current = getOverride(e);
    onApprove(e.id, { extraAllowance: Number(current.percent) || 0, extraAllowanceHours: Math.min(e.hours, Number(current.hours) || 0) });
  }
          ...
          {visible.map((e) => {
            ...
            return (
              <div className="ledger-row" key={e.id}>
                ...
                <div className="ledger-hours">{e.hours} {t("misc.hoursShort")}</div>
                <div className="ledger-loc">
                  {e.country} · {e.plant}{e.department ? ` · ${e.department}` : ""}{e.projectNumber ? ` · #${e.projectNumber}` : ""}
                </div>
                {e.status === "submitted" && isAdmin ? (
                  <div className="allowance-edit" title={t("approval.adminOverride")}>
                    <select className="input input-sm" value={override.percent} onChange={(ev) => setOverride(e.id, { percent: ev.target.value })}>
                      {ALLOWANCE_STEPS.map((p) => (
                        <option key={p} value={p}>{p}%</option>
                      ))}
                    </select>
                    {Number(override.percent) > 0 && (
                      <input
                        type="number" min="0" max={e.hours} step="0.5" className="input input-sm"
                        value={override.hours} onChange={(ev) => setOverride(e.id, { hours: ev.target.value })}
                      />
                    )}
                  </div>
                ) : (
                  e.extraAllowance > 0 && <div className="ledger-extra">+{e.extraAllowance}% ({e.extraAllowanceHours} {t("misc.hoursShort")})</div>
                )}
                {e.perDiem && <div className="ledger-extra">{t("fields.perDiem")}</div>}
                <Stamp status={e.status} t={t} />
                {e.status === "submitted" && (
                  <div className="row-actions">
                    <button className="icon-btn approve" title={t("buttons.approve")} onClick={() => approveWithOverride(e)}>
                      <Check size={15} />
                    </button>
...
function CorrectionView({ t, user, entries, employees, auditLog, onCorrect }) {
  ...
  const team = isAdminRole(user.role) ? employees.filter((e) => !isAdminRole(e.role)) : employees.filter((e) => e.supervisorId === user.id);
  const teamIds = new Set(team.map((e) => e.id));
...
function ExportView({ t, lang, entries, employees, settings, onExport }) {
  const thisMonth = new Date().toISOString().slice(0, 7);
  ...
  const rows = groupEmployees.map((emp) => {
    ...
    const perDiemDays = new Set(empEntries.filter((e) => e.perDiem).map((e) => e.date)).size;
    ...
    const extraAllowancePercent = extraAllowanceHours > 0
      ? Math.round((empEntries.reduce((s, e) => s + (e.extraAllowance || 0) * (e.extraAllowanceHours || 0), 0) / extraAllowanceHours) * 100) / 100
      : 0;
    ...
    return {
      name: emp.name, expected: expectedHours, worked, diff, perDiemDays, extraAllowancePercent, extraAllowanceHours,
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th>{t("exportView.perDiemDays")}</th>
              <th>{t("exportView.extraAllowance")}</th>
              <th>{t("exportView.extraAllowanceHours")}</th>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i}>
                ...
                <td>{r.extraAllowanceHours} {t("misc.hoursShort")}</td>
...
function UsersView({ t, employees, currentUser, isSuperAdmin, onAdd, onDelete, onUpdate }) {
                ...
                <tr key={e.id}>
                  ...
                  <td>
                    <div className="row-actions">
                      ...
                      {e.id !== currentUser.id && (e.role !== "superadmin" || isSuperAdmin) && (
...
function DepartmentsView({ t, departments, onAdd, onUpdate, onDelete }) {
  ...
  return (
    <div>
      ...
      <div className="panel" style={{ marginBottom: "1.2rem" }}>
        <div className="form-grid">
          <div className="field">
            <label className="field-label">{t("departments.name")}</label>
            <input className="input" placeholder={t("departments.namePlaceholder")} value={name} onChange={(e) => setName(e.target.value)} />
          </div>
        </div>
        {formError && <div className="login-error" style={{ marginTop: "0.6rem" }}>{formError}</div>}
        <div className="btn-row" style={{ justifyContent: "flex-start" }}>
          <button className="btn btn-primary btn-sm" disabled={!name.trim()} onClick={submit}>
          ...
          <tbody>
            {departments.map((d) => {
              ...
              if (isEditing) {
                return (
                  <tr key={d.id}>
                    <td><input className="input input-sm" value={editName} onChange={(ev) => setEditName(ev.target.value)} /></td>
                    <td>
                      <div className="row-actions">
                        <button className="icon-btn approve" title={t("buttons.save")} disabled={!editName.trim()} onClick={() => saveEdit(d.id)}>
                          <Check size={14} />
                        </button>
                        <button className="icon-btn reject" title={t("buttons.cancel")} onClick={cancelEdit}>
                          <X size={14} />
              ...
              return (
                <tr key={d.id}>
                  ...
                  <td>
                    <div className="row-actions">
                      <button className="icon-btn" title={t("buttons.edit")} onClick={() => startEdit(d)}>
                        <Pencil size={14} />
                      </button>
                      <button className="icon-btn reject" title={t("buttons.delete")} onClick={() => setConfirmingId(d.id)}>
                        <X size={14} />
                      </button>
                    </div>
                  </td>
      ...
      {confirmingId && (
        <div className="modal-backdrop" onClick={() => setConfirmingId(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <h3 className="section-title">{t("buttons.confirmDelete")}</h3>
            <p style={{ fontSize: "0.85rem" }}>{t("departments.confirmDeleteMsg")}</p>
            <p style={{ fontSize: "0.78rem", color: "var(--slate)" }}>{t("departments.deleteWarning")}</p>
            <div className="btn-row">
              <button className="btn btn-ghost" onClick={() => setConfirmingId(null)}>{t("buttons.cancel")}</button>
              <button className="btn btn-danger" onClick={() => { onDelete(confirmingId); setConfirmingId(null); }}>
                {t("buttons.delete")}
...
/* ---------------------------------------------------------------------- */
/* Entries admin view (super admin only)                                  */
/* ---------------------------------------------------------------------- */

function EntriesAdminView({ t, entries, employees, onDelete }) {
  const [empFilter, setEmpFilter] = useState("all");
  ...
  return (
    <div>
      <h2 className="section-title">{t("entriesAdmin.title")}</h2>
      <div className="filter-row">
        <select className="input input-sm" value={empFilter} onChange={(e) => setEmpFilter(e.target.value)}>
        ...
        <select className="input input-sm" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
          <option value="all">{t("entriesAdmin.all")}</option>
      ...
      {visible.length === 0 ? (
        <EmptyState>{t("entriesAdmin.empty")}</EmptyState>
      ) : (
        <div className="ledger">
          {visible.map((e) => {
            const emp = employees.find((x) => x.id === e.employeeId);
            return (
              <div className="ledger-row" key={e.id}>
                ...
                <Stamp status={e.status} t={t} />
                <div className="row-actions">
                  <button className="icon-btn reject" title={t("buttons.delete")} onClick={() => setDeletingId(e.id)}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
      ...
      {deletingId && (
        <div className="modal-backdrop" onClick={() => { setDeletingId(null); setReason(""); }}>
          <div className="modal" onClick={(ev) => ev.stopPropagation()}>
            <h3 className="section-title">{t("entriesAdmin.reasonPrompt")}</h3>
            <p style={{ fontSize: "0.78rem", color: "var(--slate)" }}>{t("entriesAdmin.confirmDeleteMsg")}</p>
            <textarea className="input" rows={3} placeholder={t("entriesAdmin.reasonPlaceholder")} value={reason} onChange={(ev) => setReason(ev.target.value)} />
            <div className="btn-row">
              <button className="btn btn-ghost" onClick={() => { setDeletingId(null); setReason(""); }}>{t("buttons.cancel")}</button>
              <button className="btn btn-danger" disabled={!reason.trim()} onClick={confirmDelete}>
                {t("buttons.delete")}
              </button>
            </div>
          </div>
...
const NAV_ICONS = { dashboard: ListChecks, newEntry: Plus, approvals: Check, corrections: FileEdit, export: FileSpreadsheet, auditLog: ClipboardList, users: Users, locations: MapPin, departments: Building2, entries: Trash2 };
...
export default function App() {
  ...
  const [settings, setSettings] = useState({ expectedMonthlyHours: 168 });
  ...
  const loadAll = useCallback(async (currentUser) => {
    const [emps, ents, sett, locs, deps] = await Promise.all([
      api.listEmployees(), api.listEntries(), api.getSettings(), api.listLocations(), api.listDepartments(),
    ]);
    setEmployees(emps);
    setEntries(ents);
    setSettings(sett);
    setLocations(locs);
    setDepartments(deps);
    if (isAdminRole(currentUser.role) || currentUser.role === "supervisor") {
  ...
  async function refreshAudit() {
    if (isAdminRole(user.role) || user.role === "supervisor") setAuditLog(await api.auditLog());
  }
  ...
  async function approveEntry(id, override) {
    try {
      await api.approveEntry(id, override);
      await refreshEntries();
    } catch (err) {
      flashToast(err.message);
    }
  ...
  async function deleteEntryHandler(id, reason) {
    try {
      await api.deleteEntry(id, reason);
      await refreshEntries();
      await refreshAudit();
      flashToast(t("misc.confirmToast"));
    } catch (err) {
    ...
    <>
      ...
      <div className="app">
        ...
        <main className="main">
          ...
          {tab === "export" && (isAdmin || isPayroll) && <ExportView t={t} lang={lang} entries={entries} employees={employees} settings={settings} onExport={doExport} />}
          ...
          {tab === "departments" && isAdmin && <DepartmentsView t={t} departments={departments} onAdd={addDepartment} onUpdate={updateDepartment} onDelete={deleteDepartment} />}
          {tab === "entries" && isSuperAdmin && <EntriesAdminView t={t} entries={entries} employees={employees} onDelete={deleteEntryHandler} />}
        </main>
      </div>
